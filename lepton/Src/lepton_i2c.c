...
uint16_t read_reg(unsigned int reg)
{
	...
	HAL_I2C_Master_Transmit(&hi2c2,LEPTON_ADDRESS,data,2,I2C_TIMEOUT);

	if( (status = HAL_I2C_Master_Receive(&hi2c2,LEPTON_ADDRESS,data,2,I2C_TIMEOUT)) == HAL_OK)
	{
		reading =  (data[0]<<8 | (data[1]));
	}
	else
	{
		printf("HAL_I2C_Master_Receive error %d\n\r",status);
	}
	return reading;
}
...
HAL_StatusTypeDef lepton_write_word(unsigned int reg,uint16_t word)
{
	...
	return HAL_I2C_Master_Transmit(&hi2c2,LEPTON_ADDRESS,data,4,I2C_TIMEOUT);
}
...
HAL_StatusTypeDef lepton_read_data(uint8_t * data)
{
	...
	{
		retrunval = HAL_I2C_Master_Receive(&hi2c2,LEPTON_ADDRESS,data,payload_length,I2C_TIMEOUT);
	}
	return retrunval;
}
...
static HAL_StatusTypeDef print_cust_serial_number()
{
  ...
  if (result != LEP_OK) {
    printf("Could not query camera customer serial number! %d\r\n", result);
    return HAL_ERROR;
  }

  printf("SYS Customer Serial Number:\r\n");
  for (i = 0; i < LEP_SYS_MAX_SERIAL_NUMBER_CHAR_SIZE; i++)
    printf("%x ", cust_serial_number.value[i]);
  printf("\r\n");
...
static HAL_StatusTypeDef print_flir_serial_number()
{
  ...
  if (result != LEP_OK) {
    printf("Could not query flir serial number! %d\r\n", result);
    return HAL_ERROR;
  }
...
static HAL_StatusTypeDef print_camera_uptime()
{
  ...
  if (result != LEP_OK) {
    printf("Could not query camera uptime! %d\r\n", result);
    return HAL_ERROR;
  }
...
static HAL_StatusTypeDef print_fpa_temp_celcius()
{
  ...
  if (result != LEP_OK) {
    printf("Could not query fpa temp! %d\r\n", result);
    return HAL_ERROR;
  }

  printf("SYS fpa temp celcius:\r\n");
...
HAL_StatusTypeDef get_scene_stats(uint16_t *min, uint16_t *max, uint16_t *avg)
{
  ...
  if (result != LEP_OK) {
    printf("Could not get scene statistics! %d\r\n", result);
    return HAL_ERROR;
  }
...
HAL_StatusTypeDef enable_telemetry(void)
{
  ...
  if (result != LEP_OK) {
    printf("Could not set telemetry location %d\r\n", result);
    return HAL_ERROR;
  }

  result = LEP_SetSysTelemetryEnableState(&hport_desc, LEP_TELEMETRY_ENABLED);
  if (result != LEP_OK) {
    printf("Could not enable telemetry %d\r\n", result);
    return HAL_ERROR;
  }
...
//  if (result != HAL_OK) {
//    printf("Could not set video output mode %d\r\n", result);
//    return HAL_ERROR;
//  }

//  result = LEP_SetVidPcolorLut(&hport_desc, pcolor_lut);
//  if (result != LEP_OK) {
//    printf("Could not set color lut: %d\r\n", result);
//    return HAL_ERROR;
//  }
...
//		hport_desc.portID = 0;
  ...
  result = LEP_OpenPort(0, LEP_CCI_TWI, 100, &hport_desc);
  if (result != LEP_OK) {
    printf("Could not open Lepton I2C port! %d\r\n", result);
    return HAL_ERROR;
  }

  printf("Lepton I2C command interface opened, device %02x\r\n", hport_desc.deviceAddress);
  ...
  if (print_aux_temp_celcius() != HAL_OK)
	...
	printf("Lepton SDK Interface OK!\r\n");
  return HAL_OK;
}
